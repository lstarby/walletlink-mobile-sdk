...
class SessionStore(private val store: StoreInterface) {
    ...
    */
     ...
     */
    fun save(rpcUrl: URL, sessionId: String, name: String, secret: String) {
        accessLock.withLock {
     ...
     * @param sessionId Session ID generated by the host
     */
    fun delete(rpcUrl: URL, sessionId: String) {
        accessLock.withLock {
     ...
     */
    fun observeSessions(url: URL): Observable<List<Session>> {
        return store.observe(StoreKeys.sessions)
    ...
    private fun getStoredSessions(): List<Session> {
        return accessLock.withLock { store.get(StoreKeys.sessions)?.toList() ?: listOf() }.sortedBy { it.id }
    }
}
